...
void mmu_init_pd(mmu_context_t* page_directory) {
    ...
    for (uint32_t addr = 0; addr < 0x40000000; addr += PAGE_SIZE) {
        ...
        if (should_map) {
            if (!mmu_map_page(page_directory, higher_half_base + addr, PAGE_SIZE, addr, PAGE_PRESENT | PAGE_RW)) {
                kprintf("Failed to map 0x%p\n", addr);
                cli(); for(;;) hlt();
            }
        } else {
            if (!mmu_map_page(page_directory, higher_half_base + addr, PAGE_SIZE, addr, 0x0)) {
                kprintf("Failed to map 0x%p not present!\n", addr);
            }
        }
    }
    mmu_map_page(page_directory, (uintptr_t)__text_start, __text_end - __text_start, (uintptr_t)__text_start - higher_half_base, PAGE_PRESENT);
